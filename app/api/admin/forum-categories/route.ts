...
export async function GET() {
  try {
    ...
    const categories = await prisma.forumCategory.findMany({
      ...
      select: { id: true, name: true, slug: true, description: true, icon: true, order: true, _count: { select: { threads: true } } },
    });
...
if (Object.keys(body).length === 0) return NextResponse.json({ error: "Dữ liệu không hợp lệ." }, { status: 400 });
    ...
    const category = await prisma.forumCategory.create({
      data: {
        name,
        slug: existing ? `${slug}-${Math.random().toString(36).slice(2, 6)}` : slug,
